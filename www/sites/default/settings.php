...
// Aussi activer l'affichage pour voir les erreurs directement
 ...
 * @endcode
 */
$settings['hash_salt'] = '_TyaaiMn-UTE4nM4j86dVZJNGiLdLG-iR10wLa_7kLbXNxitOGVeOCa9DUrNiZKT6XyGMz9yxA';
 ...
 */
// Vérifier que $app_root et $site_path sont définis avant utilisation
 ...
 */
// Configuration des hostnames autorisés pour OVH
$settings['trusted_host_patterns'] = [
  '^localhost$',
  '^127\.0\.0\.1$',
  '^::1$',
  '^.*\.local$',
  '^www\.spherevoices\.com$',
  '^spherevoices\.com$',
  '^.*\.spherevoices\.com$',
];
 ...
 * Detects production environment based on URL and loads appropriate .env file.
 */
$env_file_path = NULL;
...
if (PHP_SAPI === 'cli') {
  ...
  // For web requests, detect from HTTP_HOST
  $request_host = $_SERVER['HTTP_HOST'] ?? '';
  
  // Check if we're in production (exact match for www.spherevoices.com or spherevoices.com)
  // Using strict matching to avoid false positives (e.g., test-spherevoices.com)
  $is_production = (
    $request_host === 'www.spherevoices.com' ||
    $request_host === 'spherevoices.com' ||
    preg_match('/^(www\.)?spherevoices\.com(:[0-9]+)?$/', $request_host)
  );
  ...
  if ($env_file !== FALSE) {
    foreach ($env_file as $line) {
      ...
      if (strpos($line, '=') !== FALSE) {
        ...
        $value = trim($value, '"\'');
        // Only set if not already set in environment (environment variables take precedence).
        if (!getenv($key)) {
          putenv("$key=$value");
          $_ENV[$key] = $value;
          $_SERVER[$key] = $value;
...
// Set namespace and autoload based on driver
$driver_config = [
  'mysql' => [
...
// Configuration de la base de données de production OVH
// Les valeurs peuvent être surchargées par les variables d'environnement si définies
$databases['default']['default'] = [
  'database' => getenv('DB_NAME') ?: 'spheree921',
  'username' => getenv('DB_USER') ?: 'spheree921',
  'password' => getenv('DB_PASSWORD') ?: 'Cameroun2026',
  'prefix' => getenv('DB_PREFIX') ?: '',
  'host' => getenv('DB_HOST') ?: 'spheree921.mysql.db',
  'port' => getenv('DB_PORT') ?: ($db_driver === 'pgsql' ? '5432' : '3306'),
  'driver' => $db_driver,
];

// Add driver-specific configuration
if (isset($driver_config[$db_driver])) {
  $databases['default']['default']['namespace'] = $driver_config[$db_driver]['namespace'];
  $databases['default']['default']['autoload'] = $driver_config[$db_driver]['autoload'];
}

// MySQL-specific settings
if ($db_driver === 'mysql') {
  $databases['default']['default']['isolation_level'] = 'READ COMMITTED';
  $databases['default']['default']['collation'] = getenv('DB_COLLATION') ?: 'utf8mb4_general_ci';
}
// Configuration directory - pointant vers le répertoire versionné dans Git
// Calculer le chemin depuis settings.php (2 niveaux vers le haut pour atteindre la racine)
if (!defined('DRUPAL_ROOT')) {
    // Si DRUPAL_ROOT n'est pas défini, le calculer depuis ce fichier
    $settings['config_sync_directory'] = dirname(dirname(__DIR__)) . '/config/sync';
} else {
